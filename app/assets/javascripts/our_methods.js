...
$(document).ready(function(){
    ...
    ///////////////////////categorical histogram side by side bar chart//////////////////////
    ...
    svg.append("text")
        ...
        .attr("x", width)
        .attr("y", height + margin.top + 10)
        .text("probability distribution - original data")
        .style("font-size", "10px");
    ...
    svg.append("text")
        ...
        .attr("y", -margin.left + 10)
        .attr("x", -margin.top)
        .text("probability distribution - generated data")
        .style("font-size", "10px");
    ...
    svg.append("text").attr("x", 215).attr("y", 45).text("generated data").style("font-size", "10px").attr("alignment-baseline","middle")
    ...
    svg.append("text")
        ...
        .attr("y", -margin.left + 10)
        .attr("x", -margin.top)
        .text("accuracy score - generated data")
        .style("font-size", "10px");
    ...
    console.log(histdummy)

    for(var i=0; i<realhist.length; i++){
        var obj = {real: realhist[i], fake: fakehist[i]};
        histdummy.push(obj);
    }

    var svg = d3.select("#histogram_mockup")
                .append("svg")
                .attr("width", width + margin.left + margin.right)
                .attr("height", height + margin.top + margin.bottom)
                .append("g")
    ...
    var histogramOri = d3.histogram()
        .value(function(histdummy) { return histdummy["real"]; })
        .domain(x.domain())
        .thresholds(x.ticks(8));

    var histogramGen = d3.histogram()
        .value(function(histdummy) { return histdummy["fake"]; })
        .domain(x.domain())
        .thresholds(x.ticks(8));

    var binsOri=histogramOri(histdummy)
    var binsGen=histogramGen(histdummy)
    ...
    svg.append("circle").attr("cx",20).attr("cy",0).attr("r", 3.5).style("fill", "#fa0000").style("opacity", 0.5)
    svg.append("circle").attr("cx",20).attr("cy",15).attr("r", 3.5).style("fill", "#417ee0").style("opacity", 0.5)
    svg.append("text").attr("x", 30).attr("y", 3).text("original data").style("font-size", "10px").attr("alignment-baseline","middle")
    svg.append("text").attr("x", 30).attr("y", 18).text("generated data").style("font-size", "10px").attr("alignment-baseline","middle");


    ///MSE 3////

    emp_plot2=[];
    variables = ["LR","DTR","KNNR","SVM","RF"];
    count_real = [0.6,0.25,0.3,0.2,0.4];
    count_fake=[0.55,0.15,0.45,0.2,0.25];
    ...
    svg.append("text")
        ...
        .attr("x", width)
        .attr("y", height + margin.top + 10)
        .text("MSE(accuracy scores) - original data")
        .style("font-size", "10px");
    ...
    svg.append("text")
        ...
        .attr("y", -margin.left + 10)
        .attr("x", -margin.top)
        .text("MSE(accuracy scores) - generated data")
        .style("font-size", "10px");

    //hover function

    var tooltip = d3.select("#MSE3_plot")
                    .append("div")
                    .style("opacity", 0)
                    .attr("class", "tooltip")
                    .style("position", "absolute")
    ...
    var names = ["var13","var3","var4","var12","var7","var9","var2","var11","var10","var6","var1"]
    var x_axis = [0.1,0.6,0.3,0.3,0.6,0.5,0.7,0.6,0.7,0.8,0.4,0.95,1.0];
    var y_axis = [0.1,0.35,0.3,0.5,0.65,0.6,0.45,0.2,0.45,0.7,0.9,0.9,0.8];
    accuracy_dummy2=[];

    for(var i=0; i<x_axis.length; i++){
        var obj = {real: x_axis[i], fake: y_axis[i], variable:names[i]};
        accuracy_dummy2.push(obj);
    }

    // append the svg object to the body of the page
    var svg = d3.select("#tpredictive_mockup")
                .append("svg")
                .attr("width", width + margin.left + margin.right)
                .attr("height", height + margin.top + margin.bottom)
                .append("g")
    ...
    svg.append("text")
        ...
        .attr("x", width)
        .attr("y", height + margin.top + 10)
        .text("test metric - original data")
        .style("font-size", "10px");
    ...
    svg.append("text")
        ...
        .attr("y", -margin.left + 10)
        .attr("x", -margin.top)
        .text("test metric - generated data")
        .style("font-size", "10px");

    //hover function

    var tooltip = d3.select("#tpredictive_mockup")
                      .append("div")
                      .style("opacity", 0)
                      .attr("class", "tooltip")
                      .style("position", "absolute")
